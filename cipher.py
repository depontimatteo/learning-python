...
def parse_command_line():
    ...
    parser.add_argument('infile', type = str, help = 'usage: cipher.py [-h] [-o outfile_path] [-k KEY] [-d] [-a] [-v] infile')
    parser.add_argument('-v', '--verbose', action = 'store_true', help = 'Verbose mode')   
    parser.add_argument('-o', '--outfile', metavar = 'outfile_path', nargs = 1, type = str, default = "out.txt", help = 'output file')
    parser.add_argument('-k', '--key', metavar = 'KEY', nargs = 1, type = int, default = 1, help = 'encryption/decryption key (must be positive) (default = 1)')
    parser.add_argument('-d', '--decrypt', action = 'store_true', help = 'decrypt the input file')
    parser.add_argument('-a', '--all', action = 'store_const', const = 25, default = 1, help = 'decrypt using all keys [1, 25], save outputs in different files. (useful in case the key is lost or unknown)')
...
def transform(message, key, decrypt):
    ...
    if(decrypt):
        for c in message:
            message2 = message2 + str(shift(c, key))
    else:
        for c in message:
            message2 = message2 + str(shift(c, (key * -1)))
